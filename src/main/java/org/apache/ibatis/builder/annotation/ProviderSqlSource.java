...
public class ProviderSqlSource implements SqlSource {
	...
	private boolean				providerstaticMethod;			// 是否静态方法,在覆盖了注解类，在里面加了staticMethod属性
	...
	public ProviderSqlSource(Configuration config, Object provider) {
		...
		try {
			...
			this.providerstaticMethod = (Boolean) provider.getClass().getMethod("staticMethod").invoke(provider);
	...
	private SqlSource createSqlSource(Object parameterObject) {
		try {
			...
			if (providerTakesParameterObject)
				sql = (String) providerMethod.invoke(providerstaticMethod ? null : providerType.newInstance(), parameterObject);
			else
				sql = (String) providerMethod.invoke(providerstaticMethod ? null : providerType.newInstance());
